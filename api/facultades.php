...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        try {
            if (isset($uriPartes[2])) {
                $id = (int)$uriPartes[2];
                $stmt = $pdo->prepare("SELECT id, nombre FROM $tabla WHERE id = ?");
                $stmt->execute([$id]);
                $result = $stmt->fetch();
            } else {
    ...
    case 'POST':
        if ($tabla === 'facultades') {
            try {
                ...
                if ($nombre) {
                    $stmt = $pdo->prepare(
                        "INSERT INTO facultades (nombre) VALUES (:nombre)"
                    );
    ...
    case 'PUT':
        ...
        $estadoFacultad = $put_vars['estadoFacultad'] ?? null;

        if ($id && $nombre && $estadoFacultad !== null) {
            try {
                $stmt = $pdo->prepare("UPDATE facultades SET nombre = ?, estadoFacultad = ? WHERE id = ?");
                $stmt->execute([$nombre, $estadoFacultad, $id]);
    ...
    case 'DELETE':
        ...
        if ($id) {
            try {
                $stmt = $pdo->prepare("UPDATE facultades SET estadoFacultad = 0 WHERE id = ?");
                $stmt->execute([$id]);
